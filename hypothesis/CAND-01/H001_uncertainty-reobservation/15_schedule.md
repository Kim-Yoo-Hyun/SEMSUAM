...
- Latest schedule gate: `rival_contradiction_region_contamination_detector_promotion_gate_v1` is Docker-verified as a bounded single-case mechanism gate, `rival_contradiction_region_contamination_multi_case_source_v1` is Docker-verified as the 18-row source materializer, `rival_contradiction_region_contamination_multi_case_frame_projection_v1` is Docker-verified as the 72-row multi-case frame/projection substrate, and `rival_contradiction_region_contamination_multi_case_detector_substrate_v1` is statically frozen as the next detector/SAM2 substrate contract. The bounded chain passes detector/SAM2 substrate -> label-free post-detector evidence -> evaluation-only join -> wrong-goal/wasted-path/map-pose promotion gate on `QaLdnwvtxbs/sofa/rival_identity:3`, but this permits only `bounded_single_case_mechanism_probe_ready`. The multi-case detector/SAM2 contract consumes `72` nonblank frame rows across `9` scenes and `5` queries, fixes `min_detector_box_rate 0.8`, `min_sam2_mask_rate 0.8`, `min_candidate_association_rate 0.4`, and requires role/scene/query association diagnostics before any evidence interpretation. Terminal utility, `first_eval`, policy-scale comparison, formula revision, Step 4-5 promotion, and paper claims remain blocked until the multi-case detector/evidence/evaluation/promotion ladder passes.
