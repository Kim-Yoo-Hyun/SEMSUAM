...
- Current paper-facing blocker: paper-scale local-context detector/SAM2 substrate passes, but post-observation terminal utility is unsafe and the revised analyzer is safe-but-inert. Route-specific branch routing separates source-pool repair, goal-validity confirmation, and unresolved instance arbitration. Backend generation recovers valid candidates on `2/3` no-valid target rows, while `rival_identity:13` remains no-valid after both `spatial_nms_p80_k200_d3_v1` and `components_p80_min1_k200_v1`; that branch is a backend/source-map recall blind spot under this diagnostic scope. Candidate-specific goal-validity full substrate now passes for recovered rows `rival_identity:12` and `rival_identity:14`: detector rows `158`, detector box/SAM2/candidate association `1.0/1.0/0.9747`, and observed request/candidate rows `2/158`. Full objective analyzer observes `6` correct and `152` wrong candidates, with `146/158` candidates marked `candidate_specific_support`; the proposed objective defers both rows as support-ambiguous, while simple observed-candidate selection rules all commit wrong. The ambiguity-resolution, discriminative instance/goal-region, relation/spatial context, scene-graph/object-relation, object-relation coverage repair, object-relation observation planner, frame/projection smoke, and detector substrate gates pass, but none gives terminal utility yet. Scene-graph/object-relation target-pair separability is positive (`contrast_relation_higher 18/18`), and coverage repair sends `2` rank-6 dense relation gaps to object-relation observation while waiving `2` rank-91 medium gaps only for terminal-policy promotion. The object-relation observation substrate now has detector rows `8`, detector box/SAM2/candidate association `1.0/1.0/1.0`, and associated heading count `48`; terminal utility/policy-scale comparison remain blocked until a post-detector evidence analyzer contract separates detector availability from goal-validity utility.
...
Freeze object-relation post-detector evidence analyzer contract before terminal utility validation.

The historical runtime notes below preserve why the branch-specific route contract is now the active gate.
