...
- Current paper-facing blocker: paper-scale local-context detector/SAM2 substrate passes, but post-observation terminal utility is unsafe and the revised analyzer is safe-but-inert. Route-specific branch routing passes its Docker contract gate with `request_source_pool_repair 5`, `request_goal_validity_confirmation_evidence 7`, and `defer_instance_arbitration_unresolved 9`. Source-pool repair analyzer also passes its Docker gate, but routes all five repair rows to `request_backend_pool_expansion`. Backend pool expansion contract is now frozen, so terminal utility and policy-scale comparison remain blocked until backend expansion and goal-validity confirmation evidence are implemented and validated on a predeclared source.
...
Implement and Docker-smoke the backend pool expansion analyzer under `manifests/h001_expanded_retrieval_backend_pool_expansion_v1.json`.
