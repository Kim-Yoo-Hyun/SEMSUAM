...
- Current paper-facing blocker: independent `goal_validity_arbitration_v1` validation produced a strict-safe/inert result, while the default counterfactual produced nontrivial but unsafe commits; `discriminative_rival_view_planner_v1` v2 frame/nonblank and detector/SAM2 substrate gates pass, but `discriminative_rival_view_evidence_v1` and its failure diagnostic block threshold tuning and fresh validation. Expanded retrieval now has an analysis-only guard design, non-GT proxy feature extraction, a diagnostic source-pool validity proxy, and a standoff frame gate for detector-eligible rows. Detector/SAM2 substrate initially failed candidate association; the failure diagnostic and design gate selected `projection_anchor_height_sweep_v1`, revised observation/projection smoke passes, fixed-anchor detector/SAM2 rerun passes the substrate gate, a fresh/predeclared branch source is frozen, and fresh-source proxy/plan/frame/projection plus detector/SAM2 substrate gates pass. Fresh detector evidence diagnostic, ambiguity-aware objective contract, paper-scale source freeze, paper-scale detector observation plan, upper-anchor frame/projection smoke, paper-scale detector/SAM2 substrate, paper-scale detector evidence diagnostic, and paper-scale ambiguity objective application pass. Local-context planner/frame/detector substrate pass, but post-observation terminal utility failed. The revised analyzer repairs wrong/no-valid commit safety but is safe-and-inert, so route diagnosis is the current gate.
...
Implement and Docker-smoke paper-scale expanded-retrieval local-context disambiguation planner under `manifests/h001_expanded_retrieval_local_context_disambiguation_v1.json`.
