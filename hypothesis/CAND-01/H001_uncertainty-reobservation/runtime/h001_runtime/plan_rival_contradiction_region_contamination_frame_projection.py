...
SCHEMA_VERSION = "h001.rival_contradiction_region_contamination_frame_plan.v1"
CONTRACT_DEFAULT = (
    "hypothesis/CAND-01/H001_uncertainty-reobservation/manifests/"
    "h001_rival_contradiction_region_contamination_frame_projection_v1.json"
)
INPUT_ROOT_DEFAULT = "local_dataset/runs/h001_rival_contradiction_region_contamination_evidence_v1"
OUT_ROOT_DEFAULT = "local_dataset/runs/h001_rival_contradiction_region_contamination_frame_plan_v1"
...
POLICY_NAME = "RivalContradictionRegionContaminationEvidence"
PLANNER_NAME = "rival_contradiction_region_contamination_frame_projection_v1"
...
def select_viewpoint(
    *,
    ...
    if role in {"supported_candidate_own_view", "rival_candidate_own_view"}:
...
def build_candidate_artifact(
    *,
    ...
    for candidate_id in candidate_ids:
        role = "provisionally_supported_candidate" if candidate_id == supported_id else "rival_candidate" if candidate_id == rival_id else "context_candidate"
        candidates.append(copy_candidate(by_id[candidate_id], role))
    return {
        "schema_version": SCHEMA_VERSION,
        "artifact_type": "rival_contradiction_region_contamination_candidate_artifact",
...
def plan_row(
    *,
    ...
    target = candidates[target_id]
    context = candidates[context_id]
    candidate_ids = unique_ordered([target_id, context_id])
    ...
    return {
        ...
        "validation_stage": "rival_contradiction_region_contamination_frame_ready_plan",
        "contract_name": contract.get("contract_name"),
        "policy": POLICY_NAME,
        "planner_name": PLANNER_NAME,
        "viewpoint_policy": VIEWPOINT_POLICY,
        "viewpoint_id": f"rival_contamination:{request_id}:{role}:{row_index}",
        ...
        "viewpoint_pair_role": "supported_vs_rival_candidate_pair",
        ...
        "provisionally_supported_candidate_id": observation.get("provisionally_supported_candidate_id"),
        "rival_candidate_id": observation.get("rival_candidate_id"),
...
def skip_row(observation: Mapping[str, Any], reason: str) -> Dict[str, Any]:
    return {
        "schema_version": SCHEMA_VERSION,
        "validation_stage": "rival_contradiction_region_contamination_frame_plan_skipped",
        "policy": POLICY_NAME,
        "planner_name": PLANNER_NAME,
        "skip_reason": reason,
        "episode_key": observation.get("episode_key"),
...
def scan_forbidden_keys(value: Any, prefix: str = "") -> List[str]:
    ...
    if isinstance(value, dict):
        for key, child in value.items():
            ...
            if lowered not in {"uses_gt_for_action", "uses_gt_for_analysis"} and any(
...
def build_summary(
    *,
    ...
    expected_rows = int(((contract.get("frame_projection_contract") or {}).get("expected_decision_rows")) or 4)
    required_roles = set((contract.get("frame_projection_contract") or {}).get("required_observation_roles") or [])
    forbidden = forbidden_findings([*plan_rows, *skipped_rows, candidate_artifact])
    ...
    gate = {
        ...
        "candidate_artifact_rows_passed": candidate_artifact.get("candidate_count") == 2,
        "action_evidence_forbidden_key_gate_passed": len(forbidden) == 0,
        "terminal_commit_rows_passed": len(terminal_rows) == 0,
        "uses_gt_for_action_passed": all(row.get("uses_gt_for_action") is False for row in [*plan_rows, *skipped_rows]),
        "paper_claim_allowed_true_rows_passed": all(row.get("paper_claim_allowed") is False for row in [*plan_rows, *skipped_rows]),
        "uses_gt_for_action": False,
        "paper_claim_allowed": False,
    }
    gate["rival_contradiction_region_contamination_frame_plan_gate_passed"] = all(
    ...
    return {
        ...
        "observation_role_counts": compact_counter(row.get("observation_role") for row in plan_rows),
        ...
        "gate": gate,
        "output_files": {
            "frame_plan_rows": "rival_contradiction_region_contamination_frame_plan_rows.jsonl",
            "candidate_artifact": "rival_contradiction_region_contamination_candidate_artifact.jsonl",
            "skipped_rows": "rival_contradiction_region_contamination_frame_plan_skipped_rows.jsonl",
            "summary": "rival_contradiction_region_contamination_frame_plan_summary.json",
        },
        "interpretation": {
            "fact": "The planner resolves symbolic supported-vs-rival observation roles into frame-ready Habitat viewpoints using local-context candidate geometry.",
            "agent_inference": "Passing this gate means the four nonterminal observation roles are renderable inputs for a frame/projection smoke; it does not establish detector evidence or terminal ObjectNav utility.",
...
def run(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    observations = load_jsonl(input_root / "rival_contradiction_region_contamination_observation_plan_rows.jsonl")
    geometry = geometry_index(Path(args.geometry_candidate_artifact))
    candidate_ids = unique_ordered(
        [value for row in observations for value in (row.get("target_candidate_id"), row.get("context_candidate_id"))]
    )
    ...
    try:
        for row_index, observation in enumerate(observations, start=1):
            scene_key = str(observation.get("scene_key"))
            query = str(observation.get("query"))
            source = geometry.get((scene_key, query))
            if source is None:
                skipped_rows.append(skip_row(observation, "missing_geometry_source"))
                continue
            supported_id = str(observation.get("provisionally_supported_candidate_id"))
            rival_id = str(observation.get("rival_candidate_id"))
            if candidate_artifact is None:
                candidate_artifact = build_candidate_artifact(
                    source=source,
                    candidate_ids=sorted(candidate_ids, key=candidate_sort_key),
                    supported_id=supported_id,
                    rival_id=rival_id,
                )
            candidates = {str(candidate.get("candidate_id")): dict(candidate) for candidate in candidate_artifact.get("candidates") or []}
    ...
    write_jsonl(out_root / "rival_contradiction_region_contamination_frame_plan_rows.jsonl", plan_rows)
    write_jsonl(out_root / "rival_contradiction_region_contamination_candidate_artifact.jsonl", [candidate_artifact])
    write_jsonl(out_root / "rival_contradiction_region_contamination_frame_plan_skipped_rows.jsonl", skipped_rows)
    write_json(out_root / "rival_contradiction_region_contamination_frame_plan_summary.json", summary)
    return summary


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Resolve rival contradiction / region contamination rows into frame-ready Habitat viewpoints.")
    ...
    parser.add_argument("--run-id", default="h001_rival_contradiction_region_contamination_frame_projection_v1")
...
def main() -> None:
    ...
    if not summary["gate"]["rival_contradiction_region_contamination_frame_plan_gate_passed"]:
        raise SystemExit(1)
