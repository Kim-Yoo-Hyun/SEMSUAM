...
SCHEMA_VERSION = "h001.goal_region_object_relation_coverage_completion_frame_plan.v1"
CONTRACT_DEFAULT = (
    "hypothesis/CAND-01/H001_uncertainty-reobservation/manifests/"
    "h001_goal_region_object_relation_coverage_completion_frame_projection_v1.json"
)
INPUT_ROOT_DEFAULT = "local_dataset/runs/h001_goal_region_object_relation_coverage_completion_v1"
OUT_ROOT_DEFAULT = "local_dataset/runs/h001_goal_region_object_relation_coverage_completion_frame_plan_v1"
...
POLICY_NAME = "GoalRegionObjectRelationCoverageCompletion"
PLANNER_NAME = "goal_region_object_relation_coverage_completion_frame_projection_v1"
...
OUTPUT_FILES = {
    "frame_plan_rows": "goal_region_object_relation_coverage_completion_frame_plan_rows.jsonl",
    "candidate_artifact": "goal_region_object_relation_coverage_completion_candidate_artifact.jsonl",
    "skipped_rows": "goal_region_object_relation_coverage_completion_frame_plan_skipped_rows.jsonl",
    "summary": "goal_region_object_relation_coverage_completion_frame_plan_summary.json",
}
...
def request_id(row: Mapping[str, Any]) -> str:
    return str(row.get("request_id") or row.get("expanded_retrieval_request_id") or "")
...
def geometry_index(path: Path) -> Dict[Tuple[str, str], Dict[str, Any]]:
    ...
    for row in load_jsonl(path):
        indexed[(str(row.get("scene_key")), str(row.get("query")))] = row
    return indexed
...
def select_viewpoint(
    *,
    ...
    if role in {"candidate_a_goal_region_context_view", "candidate_b_goal_region_context_view"}:
    ...
    if candidate_a_id not in candidates or candidate_b_id not in candidates:
        return None
    preferred_side = "left" if role == "shared_goal_region_anchor_view" else "right"
...
def candidate_role_for(candidate_id: str, pair_roles: Sequence[Mapping[str, Any]], original_role: Any) -> str:
    ...
    for row in pair_roles:
        if str(row.get("candidate_id")) == candidate_id:
            role = str(row.get("candidate_pair_role") or row.get("candidate_role") or row.get("role_scope") or "")
            if role and role not in roles:
                roles.append(role)
    original = str(original_role or "")
    if original and original not in roles:
        roles.append(original)
    return "+".join(roles) if roles else "coverage_completion_pair_candidate"
        ...
        artifacts.append(
            {
                ...
                "artifact_type": "goal_region_object_relation_coverage_completion_candidate_artifact",
...
def plan_row(
    *,
    ...
    rid = request_id(observation)
    role = str(observation.get("observation_role"))
    return {
        "schema_version": SCHEMA_VERSION,
        "run_id": str(args.run_id),
        "validation_stage": "goal_region_object_relation_coverage_completion_frame_ready_plan",
        "contract_name": contract.get("contract_name"),
        "policy": POLICY_NAME,
        "planner_name": PLANNER_NAME,
        "viewpoint_policy": VIEWPOINT_POLICY,
        "viewpoint_id": f"coverage_completion:{rid}:{role}:{row_index}",
        "coverage_completion_pair_id": observation.get("coverage_completion_pair_id"),
        ...
        "coverage_completion_observation_role": role,
        "evidence_axis": observation.get("evidence_axis"),
        "selected_nonterminal_action": observation.get("selected_nonterminal_action"),
        "viewpoint_pair_role": "goal_region_object_relation_coverage_completion_candidate_pair",
...
def skip_row(observation: Mapping[str, Any], reason: str) -> Dict[str, Any]:
    return {
        "schema_version": SCHEMA_VERSION,
        "validation_stage": "goal_region_object_relation_coverage_completion_frame_plan_skipped",
        "policy": POLICY_NAME,
        "planner_name": PLANNER_NAME,
        "skip_reason": reason,
        "coverage_completion_pair_id": observation.get("coverage_completion_pair_id"),
        ...
        "request_id": request_id(observation),
...
def build_summary(
    *,
    ...
    expected_rows = int(frame_contract.get("expected_symbolic_observation_seed_rows") or frame_contract.get("minimum_frame_plan_rows") or 48)
    expected_pair_rows = int(frame_contract.get("expected_target_pair_rows") or source_gate.get("target_pair_rows") or 12)
    ...
    gate = {
        "expected_target_pair_rows_passed": len({row.get("coverage_completion_pair_id") for row in observations}) == expected_pair_rows,
    ...
    gate["goal_region_object_relation_coverage_completion_frame_plan_gate_passed"] = all(
    ...
    return {
        ...
        "coverage_completion_pair_rows": len({row.get("coverage_completion_pair_id") for row in observations}),
        ...
        "nonterminal_action_counts": compact_counter(row.get("selected_nonterminal_action") for row in plan_rows),
        ...
        "interpretation": {
            "fact": "The planner resolves 48 symbolic coverage-completion observation roles into frame-ready Habitat viewpoints using local-context candidate geometry.",
            "agent_inference": "Passing this gate means the frozen missing-evidence observation roles are renderable inputs for a frame/projection smoke; it does not establish detector evidence, wrong-goal reduction, wasted-path reduction, or map/pose consistency utility.",
...
def run(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    observations = load_jsonl(input_root / "goal_region_object_relation_coverage_completion_observation_seed_rows.jsonl")
    candidate_role_rows = load_jsonl(input_root / "goal_region_object_relation_coverage_completion_candidate_rows.jsonl")
    geometry = geometry_index(Path(args.geometry_candidate_artifact))
    candidate_artifacts, artifact_missing = build_candidate_artifacts(
    ...
    write_jsonl(out_root / OUTPUT_FILES["frame_plan_rows"], plan_rows)
    write_jsonl(out_root / OUTPUT_FILES["candidate_artifact"], candidate_artifacts)
    write_jsonl(out_root / OUTPUT_FILES["skipped_rows"], skipped_rows)
    write_json(out_root / OUTPUT_FILES["summary"], summary)
    return summary


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Resolve goal-region/object-relation coverage-completion rows into frame-ready Habitat viewpoints.")
    ...
    parser.add_argument("--run-id", default="h001_goal_region_object_relation_coverage_completion_frame_projection_v1")
...
def main() -> None:
    ...
    if not summary["gate"]["goal_region_object_relation_coverage_completion_frame_plan_gate_passed"]:
        raise SystemExit(1)
