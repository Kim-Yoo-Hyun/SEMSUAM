...
CONTRACT=${CONTRACT:-${HYP}/manifests/h001_expanded_retrieval_deeper_backend_generation_v1.json}
BACKEND_GENERATION_ROWS=${BACKEND_GENERATION_ROWS:-${RUNS_ROOT}/h001_expanded_retrieval_backend_candidate_generation_v1/backend_candidate_generation_rows.jsonl}
BACKEND_GENERATION_EVALUATED_ROWS=${BACKEND_GENERATION_EVALUATED_ROWS:-${RUNS_ROOT}/h001_expanded_retrieval_backend_candidate_generation_v1/backend_candidate_generation_evaluated_rows.jsonl}
...
OUT_ROOT=${OUT_ROOT:-${RUNS_ROOT}/h001_expanded_retrieval_deeper_backend_generation_v1}
SCENE_SPECS_FILE=${SCENE_SPECS_FILE:-${OUT_ROOT}/deeper_backend_scene_specs.txt}
ARTIFACT_OUT=${ARTIFACT_OUT:-${RUNS_ROOT}/h001_expanded_retrieval_deeper_backend_artifact_spatial_nms_p90_k100_d5_v1}
CANDIDATE_ARTIFACT=${CANDIDATE_ARTIFACT:-${ARTIFACT_OUT}/all_scenes_aligned.jsonl}
LOG=${LOG:-${HYP}/runtime/logs/expanded-retrieval-deeper-backend-generation-${TS}.log}
...
ARTIFACT_ID=${ARTIFACT_ID:-expanded_retrieval_deeper_backend_spatial_nms_p90_k100_d5_v1}
TRAJECTORY_SUFFIX=${TRAJECTORY_SUFFIX:-expanded_retrieval_deeper_backend_spatial_nms_p90_k100_d5_v1}
FRAMES=${FRAMES:-256}
WIDTH=${WIDTH:-320}
HEIGHT=${HEIGHT:-240}
DEPTH_SAMPLE_RATE=${DEPTH_SAMPLE_RATE:-8}
TOP_PERCENTILE=${TOP_PERCENTILE:-90.0}
MAX_CANDIDATES=${MAX_CANDIDATES:-100}
CANDIDATE_SELECTION_MODE=${CANDIDATE_SELECTION_MODE:-spatial_nms}
SPATIAL_NMS_MIN_DISTANCE_CELLS=${SPATIAL_NMS_MIN_DISTANCE_CELLS:-5.0}
MIN_COMPONENT_CELLS=${MIN_COMPONENT_CELLS:-1}
EXPECTED_SCENE_COUNT=${EXPECTED_SCENE_COUNT:-2}
EXPECTED_QUERY_ROWS=${EXPECTED_QUERY_ROWS:-12}
...
payload = {
    "schema_version": "h001.expanded_retrieval_deeper_backend_generation_job.v1",
    "status": sys.argv[2],
    "stage": sys.argv[3],
    "updated_at": datetime.now(timezone.utc).isoformat(),
    "working_directory": "${ROOT}",
    "command": "TS=${TS} bash ${HYP}/runtime/jobs/expanded_retrieval_deeper_backend_generation.sh",
    "contract": "${CONTRACT}",
    "backend_generation_rows": "${BACKEND_GENERATION_ROWS}",
    "backend_generation_evaluated_rows": "${BACKEND_GENERATION_EVALUATED_ROWS}",
    ...
    "expected_files": [
        ...
        "${CANDIDATE_ARTIFACT}",
        "${OUT_ROOT}/deeper_backend_generation_rows.jsonl",
        "${OUT_ROOT}/deeper_backend_generation_evaluated_rows.jsonl",
        "${OUT_ROOT}/deeper_backend_generation_summary.json",
        "${OUT_ROOT}/deeper_backend_scene_query_artifacts.jsonl"
    ],
    "verification_command": "cat ${PIPELINE_STATUS} && cat ${ARTIFACT_OUT}/coverage_check.json && cat ${OUT_ROOT}/deeper_backend_generation_summary.json",
...
job: expanded_retrieval_deeper_backend_generation
working_directory: ${ROOT}
command: TS=${TS} bash ${HYP}/runtime/jobs/expanded_retrieval_deeper_backend_generation.sh
contract: ${CONTRACT}
backend_generation_rows: ${BACKEND_GENERATION_ROWS}
...
verification_command:
  ...
  cat ${OUT_ROOT}/deeper_backend_generation_summary.json
EOF
...
echo "verification_command=cat ${PIPELINE_STATUS} && cat ${OUT_ROOT}/deeper_backend_generation_summary.json"
...
paths = {
    ...
    "backend_generation_rows": Path("${BACKEND_GENERATION_ROWS}"),
    "backend_generation_evaluated_rows": Path("${BACKEND_GENERATION_EVALUATED_ROWS}"),
    ...
    "analyzer": Path("${HYP}/runtime/h001_runtime/analyze_expanded_retrieval_deeper_backend_generation.py"),
...
docker run --rm --ipc=host \
  ...
  micromamba run -n base python -m h001_runtime.analyze_expanded_retrieval_deeper_backend_generation \
    --contract "/workspace/hypothesis/CAND-01/H001_uncertainty-reobservation/manifests/$(basename "${CONTRACT}")" \
    --backend-generation-rows "/runs/$(basename "$(dirname "${BACKEND_GENERATION_ROWS}")")/$(basename "${BACKEND_GENERATION_ROWS}")" \
    --backend-generation-evaluated-rows "/runs/$(basename "$(dirname "${BACKEND_GENERATION_EVALUATED_ROWS}")")/$(basename "${BACKEND_GENERATION_EVALUATED_ROWS}")" \
    --episode-manifest "/workspace/hypothesis/CAND-01/H001_uncertainty-reobservation/manifests/$(basename "${EPISODE_MANIFEST}")" \
    --episode-manifest-split "${EPISODE_MANIFEST_SPLIT}" \
    --data-root /data \
    --scene-spec-source "/workspace/hypothesis/CAND-01/H001_uncertainty-reobservation/manifests/$(basename "${SCENE_SPEC_SOURCE}")" \
...
python - "${OUT_ROOT}/deeper_backend_generation_summary.json" <<'PY'
import json
import sys
from pathlib import Path
...
print(json.dumps({
    "deeper_backend_generation_gate_passed": summary.get("gate", {}).get("deeper_backend_generation_gate_passed"),
    ...
    "deeper_generation_status_counts": summary.get("deeper_generation_status_counts"),
