...
LOG=${LOG:-$ROOT/logs/hm3d-restore-$(date +%Y%m%d-%H%M%S).log}
STATUS="$OUT/job_status.json"

mkdir -p "$ROOT/logs" "$OUT" "$DATA_ROOT"
...
echo "verification_command=python hypothesis/CAND-01/H001_uncertainty-reobservation/runtime/check_hm3d.py $DATA_ROOT"
...
write_status running "$CURRENT_STAGE"
python hypothesis/CAND-01/H001_uncertainty-reobservation/runtime/check_hm3d.py "$DATA_ROOT" > "$OUT/check_hm3d.json"
