...
MODEL_DIR=${MODEL_DIR:-/tmp/research3-models/openvocab/owlvit/google_owlvit-base-patch32}
OUT=${OUT:-/tmp/research3-runs/openvocab_perception_owlvit_setup}
LOG=${LOG:-$ROOT/logs/openvocab-perception-owlvit-setup-$(date +%Y%m%d-%H%M%S).log}
STATUS="$OUT/job_status.json"

mkdir -p "$ROOT/logs" "$OUT" "$MODEL_DIR"
exec > >(tee -a "$LOG") 2>&1
...
sg docker -c "docker build \
  -f hypothesis/CAND-01/H001_uncertainty-reobservation/runtime/Dockerfile.openvocab-perception \
  -t $IMAGE \
