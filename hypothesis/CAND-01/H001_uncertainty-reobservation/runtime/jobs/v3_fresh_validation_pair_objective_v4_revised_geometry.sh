...
LOG=${LOG:-${HYP}/runtime/logs/v3-fresh-validation-pair-objective-v4-revised-geometry-${TS}.log}
STATUS=${STATUS:-${PAIR_OUT}/pipeline_status.json}
DEVICE=${DEVICE:-cuda}
EPISODES=${EPISODES:-100}
RUN_PREFIX=${RUN_PREFIX:-h001_v3_fresh_validation_v1_v4_revised_geometry}

mkdir -p "${HYP}/runtime/logs" "${PAIR_OUT}"
exec > >(tee -a "${LOG}") 2>&1
...
LOG="${HYP}/runtime/logs/v3-fresh-validation-pair-v4-revised-geometry-substrate-${TS}.log" \
STATUS="${PAIR_OUT}/job_status.json" \
"${HYP}/runtime/jobs/first_eval_replacement_pair_objective_v2_substrate.sh"
