...
SOURCE_NAME="${SOURCE_NAME:-h001_rival_contradiction_region_contamination_multi_case_source_v1}"
GEOMETRY_NAME="${GEOMETRY_NAME:-h001_expanded_retrieval_paper_scale_local_context_plan_v1}"
PLAN_OUT="${PLAN_OUT:-${RUNS_ROOT}/h001_rival_contradiction_region_contamination_multi_case_frame_plan_v1}"
FRAME_OUT="${FRAME_OUT:-${RUNS_ROOT}/h001_rival_contradiction_region_contamination_multi_case_frames_v1}"
PROJECTION_OUT="${PROJECTION_OUT:-${RUNS_ROOT}/h001_rival_contradiction_region_contamination_multi_case_projection_v1}"
STATUS="${STATUS:-${PROJECTION_OUT}/frame_projection_job_status.json}"

EXPECTED_FRAME_ROWS="${EXPECTED_FRAME_ROWS:-72}"
...
POLICY="${POLICY:-RivalContradictionRegionContaminationEvidenceMultiCase}"
...
PLAN_ROWS_FILE="rival_contradiction_region_contamination_multi_case_frame_plan_rows.jsonl"
CANDIDATE_ARTIFACT_FILE="rival_contradiction_region_contamination_multi_case_candidate_artifact.jsonl"
...
docker run --rm --ipc=host ${DOCKER_GPU_ARGS} ${DOCKER_USER_ARGS} \
  ...
  -w /workspace \
  "${HABITAT_IMG}" \
  "${PYTHON_BIN}" -m h001_runtime.plan_rival_contradiction_region_contamination_multi_case_frame_projection \
    --data-root /data \
