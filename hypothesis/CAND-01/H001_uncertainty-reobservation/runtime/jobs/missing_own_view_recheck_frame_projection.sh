...
PLAN_OUT="${PLAN_OUT:-${RUNS_ROOT}/h001_missing_own_view_recheck_observation_v1}"
FRAME_OUT="${FRAME_OUT:-${RUNS_ROOT}/h001_missing_own_view_recheck_observation_frames_v1}"
PROJECTION_OUT="${PROJECTION_OUT:-${RUNS_ROOT}/h001_missing_own_view_recheck_observation_projection_v1}"
STATUS="${STATUS:-${PROJECTION_OUT}/frame_projection_job_status.json}"

EXPECTED_FRAME_ROWS="${EXPECTED_FRAME_ROWS:-80}"
...
POLICY="${POLICY:-MissingOwnViewRecheckObservation}"
...
docker run --rm --ipc=host ${DOCKER_GPU_ARGS} ${DOCKER_USER_ARGS} \
  ...
  "${PYTHON_BIN}" -m h001_runtime.export_postview_frames_v2 \
    --data-root /data \
    --viewpoint-decisions /runs/h001_missing_own_view_recheck_observation_v1/missing_own_view_observation_plan.jsonl \
    --candidate-artifact /runs/h001_missing_own_view_recheck_observation_v1/missing_own_view_candidate_artifact.jsonl \
    --out-root /runs/h001_missing_own_view_recheck_observation_frames_v1 \
    --policy "${POLICY}" \
...
docker run --rm --ipc=host ${DOCKER_USER_ARGS} \
  ...
  "${PYTHON_BIN}" -m h001_runtime.filter_nonblank_frame_summary \
    --frame-summary /runs/h001_missing_own_view_recheck_observation_frames_v1/rival_identity_frame_summary.jsonl \
    --frame-root /runs/h001_missing_own_view_recheck_observation_frames_v1 \
    --out-root /runs/h001_missing_own_view_recheck_observation_frames_v1/nonblank_filter_v1 \
    --min-stddev 0.0
...
docker run --rm --ipc=host ${DOCKER_USER_ARGS} \
  ...
  "${PYTHON_BIN}" -m h001_runtime.smoke_expanded_retrieval_projection_anchor \
    --frames /runs/h001_missing_own_view_recheck_observation_frames_v1/nonblank_filter_v1/rival_identity_frame_summary_nonblank.jsonl \
    --frame-root /runs/h001_missing_own_view_recheck_observation_frames_v1 \
    --candidate-artifact /runs/h001_missing_own_view_recheck_observation_v1/missing_own_view_candidate_artifact.jsonl \
    --out-root /runs/h001_missing_own_view_recheck_observation_projection_v1 \
