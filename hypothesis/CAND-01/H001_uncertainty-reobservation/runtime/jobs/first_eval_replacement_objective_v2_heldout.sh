...
LOG=${LOG:-${HYP}/runtime/logs/first-eval-replacement-objective-v2-heldout-${TS}.log}
STATUS=${STATUS:-${EXTERNAL_OUT}/objective_v2_heldout_job_status.json}

mkdir -p "${HYP}/runtime/logs" "${PAIR_OUT}" "${EXTERNAL_OUT}" "${EXTERNAL_EVIDENCE_V4_OUT}"
exec > >(tee -a "${LOG}") 2>&1
...
payload = {
    ...
    "exact_command": "TS=${TS} bash ${HYP}/runtime/jobs/first_eval_replacement_objective_v2_heldout.sh",
    "manifest": "${MANIFEST}",
...
echo "exact_command=TS=${TS} bash ${HYP}/runtime/jobs/first_eval_replacement_objective_v2_heldout.sh"
...
LOG="${HYP}/runtime/logs/first-eval-replacement-pair-v4b-heldout-${TS}.log" \
STATUS="${PAIR_OUT}/pipeline_status.json" \
DEVICE="${DEVICE}" \
EPISODES="${EPISODES}" \
RUN_PREFIX="h001_first_eval_replacement_v1_v4b_heldout" \
bash "${HYP}/runtime/jobs/v3_fresh_validation_pair_objective_v4_revised_geometry.sh"
...
LOG="${HYP}/runtime/logs/first-eval-replacement-external-detector-heldout-${TS}.log" \
STATUS="${EXTERNAL_OUT}/external_detector_job_status.json" \
DEVICE="${DEVICE}" \
EXTERNAL_BUDGET="${EXTERNAL_BUDGET}" \
EXPECTED_TRIGGERED_ROWS=any \
RUN_ID="${RUN_ID}_external_detector" \
bash "${HYP}/runtime/jobs/v3_fresh_validation_pair_objective_v4b_external_candidate_detector.sh"
...
VALIDATION_SCOPE="${VALIDATION_SCOPE}" \
LOG="${HYP}/runtime/logs/first-eval-replacement-followup-v3-stage2-objective-v2-heldout-${TS}.log" \
...
bash "${HYP}/runtime/jobs/v3_fresh_external_candidate_followup_v2_stage2.sh"
