...
LOG=${LOG:-${HYP}/runtime/logs/v3-fresh-validation-pair-v4b-external-candidate-detector-${TS}.log}
STATUS=${STATUS:-${OUT}/job_status.json}

mkdir -p "${HYP}/runtime/logs" "${OUT}" "${PLAN_OUT}" "${FRAMES_OUT}" "${DETECTOR_OUT}" "${EVIDENCE_OUT}"
exec > >(tee -a "${LOG}") 2>&1
