...
- H001은 umbrella hypothesis draft이며 active re-observation evidence gate를 진행 중이다.
- Current gate: freeze `candidate_conditioned_blocker_multi_case_validation_contract`.
- Latest verified materializer: `fully_covered_candidate_conditioned_contrast_v1`.
- Core code: `src/h001_runtime/materialize_fully_covered_candidate_conditioned_contrast.py`.
- Contract: `configs/h001/manifests/h001_fully_covered_candidate_conditioned_contrast_v1.json`.
- Result summary: `local_dataset/runs/h001_fully_covered_candidate_conditioned_contrast_v1/fully_covered_candidate_conditioned_contrast_summary.json`.
- Current result is diagnostic only: pair/candidate/candidate-role/alternative rows `2/4/16/7`, contrast gate passed, terminal utility and paper claim blocked.
- Next task is multi-case validation of the candidate-conditioned blocker before any terminal utility or policy-scale comparison.
