...
SCHEMA_VERSION = "h001.expanded_retrieval_pool_validity_branch.v1"
SOURCE_HANDOFF_ACTION = "request_non_gt_pool_validity_proxy_or_fallback_backend_variant"
FALLBACK_STATUSES = [
    "generated_pool_validity_fallback_pool",
    "request_second_fallback_backend_variant",
    "defer_pool_validity_fallback_unresolved",
]
...
def source_branch_rows(rows: Sequence[Dict[str, Any]], contract: Dict[str, Any]) -> List[Dict[str, Any]]:
    expected = set(target_request_ids(contract))
    return sorted(
        [
            row
            for row in rows
            if str(row.get("expanded_retrieval_request_id")) in expected
            and row.get("handoff_action") == SOURCE_HANDOFF_ACTION
        ],
...
def write_scene_specs(
    ...
    for offset, scene_key in enumerate(sorted(by_scene), start=1):
        line = scene_spec_line(by_scene[scene_key], known_specs, fallback_seed=202605291 + offset)
        lines.append(line)
        scene_lines[scene_key] = line
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    if not scene_lines:
        raise ValueError("no scene specs generated for pool-validity fallback branch")
    return scene_lines


def previous_pool_ids(branch_row: Dict[str, Any], deeper_row: Optional[Dict[str, Any]]) -> List[str]:
    branch_ids = (branch_row.get("candidate_pool_accounting") or {}).get("deeper_candidate_ids") or []
    if branch_ids:
        return [str(value) for value in branch_ids]
    if deeper_row is None:
        return []
    return [
        str(value)
        for value in (deeper_row.get("deeper_pool_accounting") or {}).get("deeper_generated_candidate_ids") or []
    ]


def source_pool_summary(branch_row: Dict[str, Any], deeper_row: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    accounting = dict((branch_row.get("candidate_pool_accounting") or {}))
    deeper_accounting = dict((deeper_row or {}).get("deeper_pool_accounting") or {})
    return {
        "source_variant": ((branch_row.get("source_deeper_backend_config") or {}).get("variant_name")),
        "candidate_count": safe_int(
            accounting.get("deeper_candidate_count", deeper_accounting.get("deeper_generated_candidate_count"))
        ),
        "new_beyond_top20_count": safe_int(
            accounting.get("new_beyond_top20_count", deeper_accounting.get("new_beyond_top20_count"))
        ),
        "reachable_or_standoff_candidate_count": safe_int(
            accounting.get(
                "reachable_or_standoff_candidate_count",
                deeper_accounting.get("reachable_or_standoff_candidate_count"),
            )
        ),
        "positive_support_candidate_count": safe_int(
            accounting.get("positive_support_candidate_count", deeper_accounting.get("positive_support_candidate_count"))
        ),
        "duplicate_candidate_id_count": safe_int(
            accounting.get("duplicate_candidate_id_count", deeper_accounting.get("duplicate_candidate_id_count"))
        ),
        "nonfinite_candidate_position_count": safe_int(
            accounting.get(
                "nonfinite_candidate_position_count",
                deeper_accounting.get("nonfinite_candidate_position_count"),
            )
        ),
        "candidate_ids": previous_pool_ids(branch_row, deeper_row),
    }


def fallback_accounting(previous_ids: Sequence[str], candidates: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
    previous_set = set(str(value) for value in previous_ids)
    generated_ids = [
        str(candidate.get("candidate_id")) for candidate in candidates if candidate.get("candidate_id") is not None
    ]
    ...
    return {
        "previous_pool_candidate_count": len(previous_ids),
        "previous_pool_candidate_ids_preview": list(previous_ids)[:20],
        "fallback_generated_candidate_count": len(candidates),
        "fallback_generated_candidate_ids": generated_ids,
        "retained_from_previous_pool_count": len(previous_set & generated_set),
        "retained_from_previous_pool_ids_preview": sorted(previous_set & generated_set)[:20],
        "new_beyond_previous_pool_count": len(generated_set - previous_set),
        "new_beyond_previous_pool_ids_preview": sorted(generated_set - previous_set)[:20],
...
def fallback_decision(
    ...
    if artifact_row is None:
        return {
            "fallback_generation_status": "defer_pool_validity_fallback_unresolved",
            "fallback_generation_reason": "fallback_candidate_artifact_row_missing",
            "fallback_generation_signals": ["candidate_artifact_row_missing"],
            "terminal_commit": False,
        }
    signals: List[str] = []
    if safe_int(accounting.get("fallback_generated_candidate_count")) < safe_int(
        gates.get("fallback_candidate_count_minimum_per_request")
    ):
        signals.append("fallback_pool_below_candidate_count_minimum")
    if safe_int(accounting.get("new_beyond_previous_pool_count")) < safe_int(
        gates.get("fallback_new_beyond_previous_pool_minimum")
    ):
        signals.append("fallback_pool_has_insufficient_new_candidates_beyond_previous_pool")
    if safe_int(accounting.get("duplicate_candidate_id_count")) > safe_int(
        gates.get("duplicate_candidate_id_count_maximum")
    ):
        signals.append("fallback_pool_duplicate_candidate_ids")
    if safe_int(accounting.get("nonfinite_candidate_position_count")) > safe_int(
        ...
        signals.append("fallback_pool_nonfinite_candidate_positions")
    if safe_int(accounting.get("reachable_or_standoff_candidate_count")) <= 0:
        signals.append("fallback_pool_no_reachable_or_standoff_candidate")
    if signals:
        return {
            "fallback_generation_status": "request_second_fallback_backend_variant",
            "fallback_generation_reason": "first_fallback_variant_structural_gate_failed",
            "fallback_generation_signals": sorted(set(signals)),
            "terminal_commit": False,
        }
    return {
        "fallback_generation_status": "generated_pool_validity_fallback_pool",
        "fallback_generation_reason": "first_fallback_variant_structural_gate_passed",
        "fallback_generation_signals": ["fallback_backend_pool_structurally_valid"],
        "terminal_commit": False,
    }
        ...
        output.append(
            {
                ...
                "fallback_variant": args.variant_name,
...
def rejected_proxy_names(contract: Dict[str, Any]) -> List[str]:
    assessment = contract.get("proxy_assessment") or {}
    return [str(row.get("name")) for row in assessment.get("rejected_proxy_families") or [] if row.get("name")]
...
def build_action_rows(
    ...
    policy = dict(contract.get("fallback_backend_policy") or {})
    variant = dict(policy.get("first_fallback_variant") or {})
    ...
    for row in branch_rows:
        ...
        previous_ids = previous_pool_ids(row, deeper_row)
        accounting = fallback_accounting(previous_ids, candidates)
        decision = fallback_decision(artifact_row, accounting, gates)
        output.append(
            {
                "schema_version": SCHEMA_VERSION,
                ...
                "source_pool_accounting": source_pool_summary(row, deeper_row),
                "proxy_assessment_action": {
                    "proxy_ready": False,
                    "decision": "use_fallback_backend_variant_before_goal_validity_confirmation",
                    "rejected_proxy_families": rejected_proxy_names(contract),
                },
                ...
                "fallback_pool_accounting": accounting,
                "fallback_generated_candidates": candidates,
                "uses_gt_for_action": False,
                **decision,
            }
        )
...
def build_evaluation_labels(
    ...
    for action_row in action_rows:
        ...
        for rank, row in enumerate(action_row.get("fallback_generated_candidates") or [], start=1):
            candidate_id = str(row.get("candidate_id"))
...
def candidate_label_summary(
    ...
    candidate_ids = list((action_row.get("fallback_pool_accounting") or {}).get("fallback_generated_candidate_ids") or [])
...
def build_evaluated_rows(
    ...
    for row in action_rows:
        ...
        output.append(
            {
                **row,
                "validation_stage": "evaluation_joined_after_fallback_generation_rows",
...
def count_by_status(rows: Sequence[Dict[str, Any]]) -> Dict[str, int]:
    counts = Counter(str(row.get("fallback_generation_status")) for row in rows)
    return {status: counts.get(status, 0) for status in FALLBACK_STATUSES}
...
def summarize(
    *,
    ...
    reason_counts = Counter(str(row.get("fallback_generation_reason")) for row in action_rows)
    signal_counts = Counter(str(signal) for row in action_rows for signal in row.get("fallback_generation_signals") or [])
    generated_counts = [
        safe_int((row.get("fallback_pool_accounting") or {}).get("fallback_generated_candidate_count"))
        for row in action_rows
    ]
    new_counts = [
        safe_int((row.get("fallback_pool_accounting") or {}).get("new_beyond_previous_pool_count"))
        for row in action_rows
    ]
    duplicate_counts = [
        safe_int((row.get("fallback_pool_accounting") or {}).get("duplicate_candidate_id_count"))
        for row in action_rows
    ]
    nonfinite_counts = [
        safe_int((row.get("fallback_pool_accounting") or {}).get("nonfinite_candidate_position_count"))
        for row in action_rows
    ]
    reachable_counts = [
        safe_int((row.get("fallback_pool_accounting") or {}).get("reachable_or_standoff_candidate_count"))
        for row in action_rows
    ]
    positive_counts = [
        safe_int((row.get("fallback_pool_accounting") or {}).get("positive_support_candidate_count"))
        for row in action_rows
    ]
    ...
    recovered_rows = sum(
        ...
        and safe_int((row.get("fallback_pool_accounting") or {}).get("new_beyond_previous_pool_count")) > 0
    )
    terminal_rows = [row for row in action_rows if row.get("terminal_commit") is True]
    forbidden = action_forbidden_keys(action_rows)
    first_variant = ((contract.get("fallback_backend_policy") or {}).get("first_fallback_variant") or {}).get(
        "variant_name"
    )
    gate = {
        ...
        "proxy_rejection_reported": all(
            (row.get("proxy_assessment_action") or {}).get("proxy_ready") is False for row in action_rows
        ),
        "fallback_backend_variant_required": bool(gates.get("fallback_backend_variant_required")),
        "first_fallback_variant_fixed": str(first_variant) == str(gates.get("first_fallback_variant")),
        "fallback_candidate_count_minimum_per_request_passed": min(generated_counts or [0])
        >= safe_int(gates.get("fallback_candidate_count_minimum_per_request")),
        "fallback_candidate_count_primary_target_passed": min(generated_counts or [0])
        >= safe_int(gates.get("fallback_candidate_count_primary_target_per_request")),
        "fallback_new_beyond_previous_pool_minimum_passed": min(new_counts or [0])
        >= safe_int(gates.get("fallback_new_beyond_previous_pool_minimum")),
        ...
        "reports_proxy_rejection": bool(action_rows),
        "reports_fallback_variant_counts": bool(status_counts),
        ...
        "second_fallback_backend_required": no_valid_rows > 0,
        "paper_claim_allowed": False,
    }
    gate["pool_validity_fallback_gate_passed"] = all(
        value is True
        ...
        not in {
            ...
            "second_fallback_backend_required",
        }
    )
    return {
        "schema_version": SCHEMA_VERSION,
        ...
        "fallback_generation_status_counts": status_counts,
        "fallback_generation_reason_counts": dict(sorted(reason_counts.items())),
        "fallback_generation_signal_counts": dict(sorted(signal_counts.items())),
        "fallback_generated_candidate_rows": total_generated,
        "fallback_generated_candidate_count_stats": count_stats(generated_counts),
        "fallback_new_beyond_previous_pool_count_stats": count_stats(new_counts),
        ...
        "recovered_valid_rows_after_fallback": recovered_rows,
        ...
        "simpler_alternative_accounting": {
            "treat_structurally_strong_p90_k100_pool_as_valid": {
                "blocked_by": "structural proxy is not separable from recovered rows",
                "proxy_ready": False,
            },
            "pass_still_no_valid_row_to_goal_validity_confirmation": {
                "blocked_by": "would mix candidate-pool recall failure with goal-validity confirmation",
                "terminal_commit_allowed": False,
            },
            "defer_still_no_valid_row_without_testing_fallback": {
                "blocked_by": "contract requires one fixed fallback backend variant before marking backend blind spot",
                "first_fallback_variant": first_variant,
            },
        },
        ...
        "interpretation": {
            "fact": (
                "The analyzer consumes one backend/pool-validity branch row and writes fallback "
                "generation rows before any GT-analysis label join."
            ),
            "agent_inference": (
                "This tests whether a fixed wider non-GT backend variant can repair the candidate "
                "pool. It is not a goal-validity confirmation rule and does not permit terminal commits."
            ),
        },
        "uses_gt_for_action": False,
        "uses_gt_for_analysis": bool(label_rows),
        "paper_claim_allowed": False,
        "output_files": {
            "branch_rows": "pool_validity_branch_rows.jsonl",
            "fallback_generation_rows": "pool_validity_fallback_generation_rows.jsonl",
            "fallback_evaluated_rows": "pool_validity_fallback_evaluated_rows.jsonl",
            "evaluation_labels": "pool_validity_evaluation_labels.jsonl",
            "scene_query_artifacts": "pool_validity_scene_query_artifacts.jsonl",
            "summary": "pool_validity_summary.json",
...
def run(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    deeper_evaluated_rows = row_index(load_jsonl(Path(args.deeper_generation_evaluated_rows)))

    branch_rows = source_branch_rows(branch_rows_all, contract)
    expected_ids = target_request_ids(contract)
    observed_ids = [str(row.get("expanded_retrieval_request_id")) for row in branch_rows]
    if observed_ids != expected_ids:
        raise ValueError(f"branch request mismatch: expected={expected_ids} observed={observed_ids}")
    observed_scene_queries = [scene_query_key(row.get("scene_key"), row.get("query")) for row in branch_rows]
    expected_scene_queries = target_scene_query_keys(contract)
    if observed_scene_queries != expected_scene_queries:
        raise ValueError(f"target scene/query mismatch: expected={expected_scene_queries} observed={observed_scene_queries}")
    missing_deeper = [request_id for request_id in expected_ids if request_id not in deeper_rows]
    if missing_deeper:
        raise ValueError(f"missing deeper generation rows for branch ids: {missing_deeper}")
    missing_evaluated = [request_id for request_id in expected_ids if request_id not in deeper_evaluated_rows]
    if missing_evaluated:
        raise ValueError(f"missing deeper evaluated rows for branch ids: {missing_evaluated}")
    non_branchable = [
        request_id
        for request_id in expected_ids
        if deeper_evaluated_rows[request_id].get("evaluation_only_no_valid_candidate_pool") is not True
    ]
    if non_branchable:
        raise ValueError(f"branch targets must be no-valid after previous label join: {non_branchable}")

    scene_spec_output = Path(args.scene_spec_output) if args.scene_spec_output else out_root / "pool_validity_scene_specs.txt"
    ...
    scene_query_rows = scene_query_artifact_rows(branch_rows, artifact_rows, scene_lines, args)
    action_rows = build_action_rows(branch_rows, deeper_rows, artifact_rows, contract, args)
    ...
    write_jsonl(out_root / "pool_validity_branch_rows.jsonl", branch_rows)
    write_jsonl(out_root / "pool_validity_fallback_generation_rows.jsonl", action_rows)
    write_jsonl(out_root / "pool_validity_evaluation_labels.jsonl", label_rows)
    write_jsonl(out_root / "pool_validity_fallback_evaluated_rows.jsonl", evaluated_rows)
    write_jsonl(out_root / "pool_validity_scene_query_artifacts.jsonl", scene_query_rows)
    write_json(out_root / "pool_validity_summary.json", summary)
    return summary


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Implement the pool-validity fallback backend branch for expanded retrieval."
    )
    ...
    parser.add_argument("--variant-name", default="spatial_nms_p80_k200_d3_v1")
