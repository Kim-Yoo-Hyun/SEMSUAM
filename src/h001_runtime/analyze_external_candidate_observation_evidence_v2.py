...
from h001_runtime.analyze_external_candidate_observation_evidence import (
...
SCHEMA_VERSION = "h001.external_candidate_observation_evidence.v2"
...
def strong_depth_evidence(candidate: Dict[str, Any], args: argparse.Namespace) -> bool:
    return bool(
        float(candidate.get("strict_association_count") or 0.0)
        >= float(args.min_strong_strict_association_count)
        and float(candidate.get("mask_hit_count") or 0.0) >= float(args.min_strong_mask_hit_count)
        and float(candidate.get("visible_count") or 0.0) >= float(args.min_strong_visible_count)
    )


def duplicate_goal_evidence(best: Dict[str, Any], second: Optional[Dict[str, Any]], args: argparse.Namespace) -> bool:
    if second is None:
        return False
    return bool(
        strong_depth_evidence(best, args)
        and strong_depth_evidence(second, args)
        and (safe_float(best.get("S_ext")) or 0.0) >= float(args.min_duplicate_score)
        and (safe_float(second.get("S_ext")) or 0.0) >= float(args.min_duplicate_second_score)
    )


def select_branch_candidate_v2(
    ...
    duplicate_strong = duplicate_goal_evidence(best, second, args)
    contrast_ok = margin >= float(args.min_commit_margin)

    guard = {
        "property_group": property_group,
        "best_strong_depth_evidence": best_strong,
        "duplicate_goal_evidence": duplicate_strong,
    ...
    elif not contrast_ok and not duplicate_strong:
        return best, second_score, margin, "defer_ambiguous_external_evidence", guard
...
def summarize(rows: List[Dict[str, Any]], detector_summary: Dict[str, Any], args: argparse.Namespace) -> Dict[str, Any]:
    commit_rows = [row for row in rows if row["external_evidence_v2_commits"]]
    wrong_commits = [row for row in commit_rows if row["external_evidence_v2_wrong_goal_commit"]]
    success_commits = [row for row in commit_rows if row["external_evidence_v2_success_commit"]]
    no_valid_commits = [row for row in commit_rows if row["external_evidence_v2_no_valid_external_commit"]]
    evidence_available = [
        row for row in rows
    ...
    strong_depth_rows = [
        row for row in rows
        if any(candidate.get("v2_strong_depth_evidence") is True for candidate in row.get("candidate_evidence") or [])
    ]
    ...
    action_counts = Counter(row["external_evidence_v2_action"] for row in rows)
    reason_counts = Counter(row["external_evidence_v2_reason"] for row in rows)
    by_label: Dict[str, Counter[str]] = defaultdict(Counter)
    by_property: Dict[str, Counter[str]] = defaultdict(Counter)
    for row in rows:
        by_label[str(row.get("label_case"))][str(row["external_evidence_v2_action"])] += 1
        by_property[str(row.get("property_group"))][str(row["external_evidence_v2_action"])] += 1
    ...
    gate["passes_external_detector_substrate_gate_v2"] = bool(
    ...
    gate["passes_external_evidence_safety_gate_v2"] = bool(
    ...
    gate["passes_external_evidence_full_gate_v2"] = bool(
        gate["passes_external_detector_substrate_gate_v2"]
        and gate["passes_external_evidence_safety_gate_v2"]
...
def run(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    for branch_id, branch in sorted(branches.items()):
        ...
        for candidate_id in branch.get("external_candidate_ids") or []:
            ...
            candidate_row["v2_strong_depth_evidence"] = strong_depth_evidence(candidate_row, args)
            candidate_rows.append(candidate_row)

        selected, second_score, margin, decision, guard = select_branch_candidate_v2(candidate_rows, query, args)
        selected = selected or {}
        action = "external_evidence_v2_commit_candidate" if decision == "commit_external_candidate" else "external_evidence_v2_defer"
        ...
        evidence_rows.append(
            {
                ...
                "score_margin": margin,
                "external_evidence_v2_action": action,
                "external_evidence_v2_reason": decision,
                "external_evidence_v2_commits": action == "external_evidence_v2_commit_candidate",
                "external_evidence_v2_success_commit": bool(action == "external_evidence_v2_commit_candidate" and selected_correct is True),
                "external_evidence_v2_wrong_goal_commit": bool(action == "external_evidence_v2_commit_candidate" and selected_correct is False),
                "external_evidence_v2_no_valid_external_commit": bool(action == "external_evidence_v2_commit_candidate" and not contains_correct),
                "v2_guard": guard,
    ...
    write_jsonl(out_root / "external_candidate_evidence_v2_rows.jsonl", evidence_rows)
    ...
    summary = {
        ...
        **stats,
        "thresholds": {
            ...
            "min_duplicate_score": float(args.min_duplicate_score),
            "min_duplicate_second_score": float(args.min_duplicate_second_score),
        },
        "objective_design": {
            "not_threshold_only": [
                "uses branch-level detector substrate instead of frame-level association rate",
                "requires strong depth-associated evidence before commit",
                "adds a small-or-cluttered property guard for plant-like categories",
                "allows duplicate-goal evidence only when multiple candidates have strong depth support",
            ],
            "failure_modes_addressed": [
                "wrong plant instance with mask/projection support but weak depth association",
                "safe bed recovery with strong strict association and large evidence margin",
                "repeated chair candidates with multiple strongly associated plausible goals",
            ],
        },
        "diagnosis": {
            "branch_role": "external_view_detector_evidence_gate_v2",
            "paper_claim_status": "diagnostic_fixed_rule_on_current_detector_artifact_not_yet_heldout_paper_claim",
        },
        "uses_gt_for_action": False,
        "uses_gt_for_analysis": any(row["uses_gt_for_analysis"] for row in evidence_rows),
        "output_files": {
            "rows": "external_candidate_evidence_v2_rows.jsonl",
            "summary": "external_candidate_evidence_v2_summary.json",
        },
    }
    write_json(out_root / "external_candidate_evidence_v2_summary.json", summary)
    return summary


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analyze H001 external-candidate detector evidence objective v2.")
    ...
    parser.add_argument("--min-duplicate-score", type=float, default=0.70)
    parser.add_argument("--min-duplicate-second-score", type=float, default=0.70)
