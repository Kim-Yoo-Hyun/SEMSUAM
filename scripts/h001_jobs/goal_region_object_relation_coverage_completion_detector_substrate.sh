...
export PLAN_OUT=${PLAN_OUT:-${RUNS_ROOT}/h001_goal_region_object_relation_coverage_completion_frame_plan_v1}
export FRAME_OUT=${FRAME_OUT:-${RUNS_ROOT}/h001_goal_region_object_relation_coverage_completion_frames_v1}
...
export FRAME_PLAN_ROWS=${FRAME_PLAN_ROWS:-${PLAN_OUT}/goal_region_object_relation_coverage_completion_frame_plan_rows.jsonl}
export CANDIDATE_ARTIFACT=${CANDIDATE_ARTIFACT:-${PLAN_OUT}/goal_region_object_relation_coverage_completion_candidate_artifact.jsonl}
export OUT=${OUT:-${RUNS_ROOT}/h001_goal_region_object_relation_coverage_completion_detector_substrate_v1}
export DETECTOR_OUT=${DETECTOR_OUT:-${OUT}/detector_v3c}
export LOG=${LOG:-${ROOT}/archive/logs/h001_runtime/goal-region-object-relation-coverage-completion-detector-substrate-${TS}.log}
export STATUS=${STATUS:-${OUT}/job_status.json}

export EXPECTED_POLICY=${EXPECTED_POLICY:-GoalRegionObjectRelationCoverageCompletion}
export EXPECTED_FRAME_ROWS=${EXPECTED_FRAME_ROWS:-48}
...
export MIN_ROWS_WITH_CANDIDATE_ASSOCIATION=${MIN_ROWS_WITH_CANDIDATE_ASSOCIATION:-20}
export MIN_ASSOCIATED_CANDIDATE_HEADING_COUNT=${MIN_ASSOCIATED_CANDIDATE_HEADING_COUNT:-24}
export MIN_ROWS_WITH_CANDIDATE_ASSOCIATION_PER_ROLE=${MIN_ROWS_WITH_CANDIDATE_ASSOCIATION_PER_ROLE:-2}
export MIN_ASSOCIATED_SCENE_COUNT=${MIN_ASSOCIATED_SCENE_COUNT:-4}
export MIN_ASSOCIATED_QUERY_COUNT=${MIN_ASSOCIATED_QUERY_COUNT:-2}
export MAX_DEBUG_IMAGES=${MAX_DEBUG_IMAGES:-180}
...
payload.update(
    {
        ...
        "coverage_completion_wrapper": "goal_region_object_relation_coverage_completion_detector_substrate.sh",
        "diagnostic": "${OUT}/goal_region_object_relation_coverage_completion_detector_substrate_diagnostic.json",
    }
)
...
run_coverage_completion_diagnostic() {
  python - <<'PY'
import json
import os
...
frame_plan_path = Path(os.environ["FRAME_PLAN_ROWS"])
source_frame_path = Path(os.environ["FRAMES"])
diagnostic_path = out / "goal_region_object_relation_coverage_completion_detector_substrate_diagnostic.json"
...
def tuple_key(row: dict[str, Any]) -> str:
...
plan_rows = read_jsonl(frame_plan_path)
source_frame_rows = read_jsonl(source_frame_path)
source_frame_by_decision = {str(row.get("decision_id")): row for row in source_frame_rows}
plan_by_viewpoint = {str(row.get("viewpoint_id")): row for row in plan_rows}


def plan_for_detector_row(row: dict[str, Any]) -> dict[str, Any]:
    source_frame = source_frame_by_decision.get(str(row.get("decision_id")), {})
    viewpoint_id = source_frame.get("viewpoint_id") or row.get("viewpoint_id")
    return plan_by_viewpoint.get(str(viewpoint_id), {})

missing_plan_join_rows = 0
role_counts = Counter()
evidence_axis_counts = Counter()
nonterminal_action_counts = Counter()
pair_counts = Counter()
role_row_association_counts = Counter()
evidence_axis_row_association_counts = Counter()
nonterminal_action_row_association_counts = Counter()
pair_row_association_counts = Counter()
...
associated_decision_ids: set[str] = set()
...
for row in frame_rows:
    plan = plan_for_detector_row(row)
    if plan is None:
        missing_plan_join_rows += 1
        plan = {}
    role = str(row.get("role") or row.get("view_role") or plan.get("observation_role") or "unknown")
    evidence_axis = str(plan.get("evidence_axis") or "unknown")
    action = str(plan.get("selected_nonterminal_action") or "unknown")
    pair_id = str(plan.get("coverage_completion_pair_id") or "unknown")
    key = tuple_key(row)
    role_counts[role] += 1
    evidence_axis_counts[evidence_axis] += 1
    nonterminal_action_counts[action] += 1
    pair_counts[pair_id] += 1
    candidate_tuple_frame_counts[key] += 1
    if row.get("has_candidate_association"):
        decision_id = str(row.get("decision_id"))
        associated_decision_ids.add(decision_id)
        role_row_association_counts[role] += 1
        evidence_axis_row_association_counts[evidence_axis] += 1
        nonterminal_action_row_association_counts[action] += 1
        pair_row_association_counts[pair_id] += 1
    ...
    action_forbidden_key_count += sum(1 for key_name in forbidden_action_keys if key_name in row)
...
role_rows_pass = {role: role_row_association_counts.get(role, 0) >= min_per_role for role in required_roles}
role_count_pass = {role: role_counts.get(role, 0) == expected_frame_rows // len(required_roles) for role in required_roles}
...
gate = {
    ...
    "plan_to_frame_join_pass": missing_plan_join_rows == 0 and len(plan_by_viewpoint) == expected_frame_rows,
...
gate["passes_coverage_completion_detector_substrate_gate"] = all(gate.values())
...
if not gate["plan_to_frame_join_pass"]:
    failure_taxonomy.append(f"plan_to_frame_join_missing:{missing_plan_join_rows}")
if not gate["rows_with_candidate_association_pass"]:
    failure_taxonomy.append(f"insufficient_associated_rows:{summary.get('rows_with_candidate_association')}_lt_{min_rows}")
...
diagnostic = {
    "schema_version": "h001.goal_region_object_relation_coverage_completion_detector_substrate_diagnostic.v1",
    "contract_name": "goal_region_object_relation_coverage_completion_detector_substrate_v1",
    "source_summary": str(summary_path),
    "frame_summary": str(frame_summary_path),
    "frame_plan_rows": str(frame_plan_path),
    "source_frame_rows": str(source_frame_path),
    "associations": str(associations_path),
    "frame_rows": len(frame_rows),
    "association_rows": len(assoc_rows),
    "plan_rows": len(plan_rows),
    "source_frame_rows_count": len(source_frame_rows),
    "missing_plan_join_rows": missing_plan_join_rows,
    ...
    "evidence_axis_counts": dict(sorted(evidence_axis_counts.items())),
    "evidence_axis_row_association_counts": dict(sorted(evidence_axis_row_association_counts.items())),
    "nonterminal_action_counts": dict(sorted(nonterminal_action_counts.items())),
    "nonterminal_action_row_association_counts": dict(sorted(nonterminal_action_row_association_counts.items())),
    "coverage_completion_pair_counts": dict(sorted(pair_counts.items())),
    "coverage_completion_pair_association_counts": dict(sorted(pair_row_association_counts.items())),
...
summary["coverage_completion_diagnostic"] = {
    ...
    "evidence_axis_row_association_counts": diagnostic["evidence_axis_row_association_counts"],
    "nonterminal_action_row_association_counts": diagnostic["nonterminal_action_row_association_counts"],
    "coverage_completion_pair_association_counts": diagnostic["coverage_completion_pair_association_counts"],
...
summary["coverage_completion_gate"] = gate
summary["paper_claim_allowed"] = False
summary["uses_gt_for_action"] = False
summary_path.write_text(json.dumps(summary, indent=2, sort_keys=True), encoding="utf-8")

print(json.dumps(diagnostic, indent=2, sort_keys=True))
if not gate["passes_coverage_completion_detector_substrate_gate"]:
    raise SystemExit(1)
PY
}

mkdir -p "${OUT}" "$(dirname "${LOG}")"
echo "coverage_completion_wrapper_started_at=$(date -Is)" >> "${LOG}"
echo "working_directory=${ROOT}" >> "${LOG}"
echo "command=TS=${TS} ROOT=${ROOT} ${HYP}/scripts/h001_jobs/goal_region_object_relation_coverage_completion_detector_substrate.sh" >> "${LOG}"
echo "output_path=${OUT}" >> "${LOG}"
echo "expected_files=${OUT}/job_status.json ${OUT}/detector_v3c/summary.json ${OUT}/expanded_retrieval_detector_associations.jsonl ${OUT}/expanded_retrieval_detector_frame_summary.jsonl ${OUT}/expanded_retrieval_detector_substrate_summary.json ${OUT}/goal_region_object_relation_coverage_completion_detector_substrate_diagnostic.json" >> "${LOG}"
echo "verification_command=cat ${OUT}/job_status.json && cat ${OUT}/expanded_retrieval_detector_substrate_summary.json && cat ${OUT}/goal_region_object_relation_coverage_completion_detector_substrate_diagnostic.json" >> "${LOG}"

write_status running detector_mask_scoring
bash "${HYP}/scripts/h001_jobs/expanded_retrieval_detector_substrate.sh"

write_status running coverage_completion_diagnostic
set +e
run_coverage_completion_diagnostic 2>&1 | tee -a "${LOG}"
diagnostic_status=${PIPESTATUS[0]}
set -e

if [[ "${diagnostic_status}" -ne 0 ]]; then
  write_status failed coverage_completion_diagnostic
  exit "${diagnostic_status}"
fi

write_status completed completed
echo "coverage_completion_wrapper_completed_at=$(date -Is)" >> "${LOG}"
