...
FOLLOWUP_EVIDENCE_OUT=${FOLLOWUP_EVIDENCE_OUT:-${EXTERNAL_OUT}/external_candidate_followup_evidence_v3}
STAGE2_PLAN_OUT=${STAGE2_PLAN_OUT:-${EXTERNAL_OUT}/external_candidate_followup_identity_stage2_v3_plan}
STAGE2_FRAMES_OUT=${STAGE2_FRAMES_OUT:-${EXTERNAL_OUT}/external_candidate_followup_identity_stage2_v3_frames}
STAGE2_DETECTOR_OUT=${STAGE2_DETECTOR_OUT:-${EXTERNAL_OUT}/external_candidate_followup_identity_stage2_v3_detector}
STAGE2_EVIDENCE_OUT=${STAGE2_EVIDENCE_OUT:-${EXTERNAL_OUT}/external_candidate_followup_identity_stage2_v3_evidence}
INTEGRATED_OUT=${INTEGRATED_OUT:-${EXTERNAL_OUT}/external_candidate_followup_v3_stage2_validation}
...
DEVICE=${DEVICE:-cuda}
RUN_ID=${RUN_ID:-h001_v3_fresh_external_candidate_followup_v3_stage2}
...
EXPECTED_STAGE2_REQUEST_ROWS=${EXPECTED_STAGE2_REQUEST_ROWS:-6}
LOG=${LOG:-${ROOT}/archive/logs/h001_runtime/v3-fresh-external-candidate-followup-v3-stage2-${TS}.log}
STATUS=${STATUS:-${EXTERNAL_OUT}/external_candidate_followup_v3_stage2_job_status.json}
...
payload = {
    ...
    "exact_command": "TS=${TS} bash ${HYP}/scripts/h001_jobs/v3_fresh_external_candidate_followup_v3_stage2.sh",
...
echo "exact_command=TS=${TS} bash ${HYP}/scripts/h001_jobs/v3_fresh_external_candidate_followup_v3_stage2.sh"
...
echo "log=${LOG}"
echo "expected_files=${STAGE2_PLAN_OUT}/external_candidate_second_stage_identity_summary.json ${STAGE2_DETECTOR_OUT}/summary.json ${STAGE2_EVIDENCE_OUT}/external_candidate_second_stage_identity_evidence_summary.json ${INTEGRATED_OUT}/external_candidate_followup_v2_stage2_validation_summary.json"
...
paths = {
    ...
    "followup_evidence_rows": Path("${FOLLOWUP_EVIDENCE_OUT}") / "external_candidate_followup_evidence_rows.jsonl",
    "followup_evidence_summary": Path("${FOLLOWUP_EVIDENCE_OUT}") / "external_candidate_followup_evidence_summary.json",
...
docker run --rm --ipc=host \
  ...
  micromamba run -n base python -m h001_runtime.summarize_followup_v2_stage2 \
    --base-validation-summary "$(to_runs_path "${EXTERNAL_OUT}")/external_candidate_followup_v2_stage2_job_summary.json" \
...
payload = {
    "schema_version": "h001.v3_fresh_external_candidate_followup_v3_stage2_job.v1",
    "followup_evidence_out": "${FOLLOWUP_EVIDENCE_OUT}",
    "stage2_plan_out": "${STAGE2_PLAN_OUT}",
    "stage2_frames_out": "${STAGE2_FRAMES_OUT}",
    "stage2_detector_out": "${STAGE2_DETECTOR_OUT}",
    ...
    "uses_gt_for_action": bool(
        stage2_plan.get("uses_gt_for_action")
    ...
    "uses_gt_for_analysis": bool(
        stage2_plan.get("uses_gt_for_analysis")
        or stage2_evidence.get("uses_gt_for_analysis")
        or integrated.get("uses_gt_for_analysis")
    ),
}
(Path("${EXTERNAL_OUT}") / "external_candidate_followup_v3_stage2_job_summary.json").write_text(
