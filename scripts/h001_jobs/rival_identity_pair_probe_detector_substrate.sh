...
DEVICE=${DEVICE:-cuda}
MAX_FRAMES=${MAX_FRAMES:-19}
EXPECTED_FRAME_ROWS=${EXPECTED_FRAME_ROWS:-19}
...
MIN_CANDIDATE_ASSOCIATION_RATE=${MIN_CANDIDATE_ASSOCIATION_RATE:-0.50}
MAX_DEBUG_IMAGES=${MAX_DEBUG_IMAGES:-180}
LOG=${LOG:-${ROOT}/archive/logs/h001_runtime/rival-identity-detector-substrate-${TS}.log}
...
echo "log=${LOG}"
echo "expected_files=${DETECTOR_OUT}/summary.json ${DETECTOR_OUT}/detector_candidate_associations.jsonl ${OUT}/rival_identity_detector_associations.jsonl ${OUT}/rival_identity_detector_substrate_summary.json"
echo "verification_command=cat ${STATUS} && cat ${OUT}/rival_identity_detector_substrate_summary.json"
...
if len(frames) != int("${EXPECTED_FRAME_ROWS}"):
    raise RuntimeError({"unexpected_frame_rows": len(frames)})
if any(row.get("uses_gt_for_action") is True for row in frames):
    raise RuntimeError("GT leakage in frame rows")
if any("rival_identity_request_id" not in row for row in frames):
    raise RuntimeError("missing rival_identity_request_id in frame rows")
...
shutil.copyfile(detector_out / "detector_candidate_associations.jsonl", out / "rival_identity_detector_associations.jsonl")
shutil.copyfile(detector_out / "frame_summary.jsonl", out / "rival_identity_detector_frame_summary.jsonl")
payload = {
    "schema_version": "h001.rival_identity_detector_substrate.v1",
    "frames": "${FRAMES}",
    "candidate_artifact": "${CANDIDATE_ARTIFACT}",
    "detector_out": "${DETECTOR_OUT}",
    ...
    "output_files": {
        ...
        "rival_identity_detector_associations": "rival_identity_detector_associations.jsonl",
        "rival_identity_detector_frame_summary": "rival_identity_detector_frame_summary.jsonl",
        "summary": "rival_identity_detector_substrate_summary.json",
    },
}
payload["gate"]["passes_detector_substrate_gate"] = all(payload["gate"].values())
(out / "rival_identity_detector_substrate_summary.json").write_text(
