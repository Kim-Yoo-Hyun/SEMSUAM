...
6. Goal-validity arbitration
   ...
   This still does not define a terminal selector. The nonterminal diagnostic report is frozen, and the current rival contradiction / region contamination path is now closed as diagnostic-only. The next observation branch `goal_region_object_relation_coverage_completion_v1` has passed materializer, frame/projection smoke, detector/SAM2 substrate, label-free evidence, evaluation-only join, and promotion gate, but it is also closed as diagnostic-only. The key blocker is `fully_covered_goal_validity_failure_v1`: one row has `goal_region_both_candidates_supported_object_relation_supported` evidence while the evaluation-only audit is `a_wrong_b_correct`. Its Docker materializer isolates target/candidate/role/candidate-view/baseline-audit rows `1/2/4/8/3` without action leakage, the asymmetry inspection finds candidate-conditioned blocker evidence for that row, and the contrast contract now fixes a fully covered `both_correct` row for false-positive audit. The next valid gate is Docker contrast materialization, not terminal utility.
