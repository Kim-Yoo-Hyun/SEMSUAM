...
- `h001_dense_conflict_v1` manifest and recall gate were implemented. Manifest verify passed with `8` rows and `8` unique episode keys. Existing-artifact recall smoke on `v3_fresh_spatial_p97_k20` passed the primary gate with `6/6` rows containing a correct candidate and recall@20 `1.0`; this verifies the gate code but does not replace the final `spatial_nms_p95_k100_d10` dense backend validation.
...
- Drive restore and no-Drive rebuild procedures were added in `docs/reproducibility.md`; `h001_dense_conflict_artifacts_spatial_nms_p95_k100_d10_v1` is excluded from high-priority backup because it is currently a failed tiny artifact and should be regenerated after NVIDIA runtime recovery.
