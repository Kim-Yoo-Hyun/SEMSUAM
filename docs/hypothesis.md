...
이 문서는 현재 연구 후보를 검증 가능한 hypothesis로 바꾸는 에이전트 workflow와 작성 규칙을 정의한다. 실제 hypothesis 내용은 `experiments/` 폴더의 해당 experiment workspace에 저장한다.

## Storage Rule

Hypothesis 관련 산출물은 `experiments/` 폴더에 저장한다.

- workflow와 작성 규칙: `docs/hypothesis.md`
- hypothesis/experiment index: `experiments/README.md`
- candidate 요약: `experiments/CAND-<number>.md`
- 개별 hypothesis workspace: `experiments/h<number>_<short-title>/`
- 작업 계획과 진행 상태: `TODO.md`

`docs/hypothesis.md`는 절차와 기준만 관리한다. 문제 정의, hypothesis, feasibility gate, first experiment shape는 해당 `experiments/` workspace 아래에 기록한다.

이 repo는 기존 `literature/CAND-01.md` 표기와 맞춰 candidate 문서를 `CAND-01`, `CAND-02`처럼 두 자리 번호로 쓴다.
...
12. 대상 candidate 요약 문서
13. 대상 experiment workspace의 `README.md`
...
- Hypothesis 단계의 probe 설계, dataset/replay 접근성 판단, baseline 후보, metric 정의는 모두 대상 `experiments/` workspace 아래에 기록한다.
...
experiments/
  README.md
  CAND-01.md
  h001_uncertainty-reobservation/
    README.md
    01_problem.md
    02_hypothesis.md
    03_feasibility.md
...
- candidate 요약 문서는 `CAND-<number>.md`를 사용한다.
- 현재 repo에서는 `CAND-01`처럼 두 자리 번호를 사용한다.
- hypothesis workspace는 `h<number>_<short-title>`을 사용한다.
...
- experiment workspace 안에서 first probe contract와 가치 판단 기준이 작성되어 있다.
