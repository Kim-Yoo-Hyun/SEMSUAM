...
최신 dense path 기준으로는, 같은-goal correct cluster에서 성공한 local chair diagnostic을 확장 주장으로 쓰지 않는다. 현재 reproducible next step은 frozen non-GT policy selector materializer를 Docker로 구현하는 것이다.
