...
현재 positive signal은 semantic uncertainty를 `commit/defer` threshold가 아니라 `request_identity_confirmation`과 `request_expanded_retrieval` 같은 active observation contract로 바꾸는 방향에서 나온다. 아직 paper claim은 확정하지 않는다. Fresh detector rerun은 V2가 compact object distractor에 취약하다는 새 failure mechanism을 확인했고, V3/V4는 이 failure를 safety repair로 막지만 separate split utility proof는 아직 없다. Broader split의 zero-standoff geometry blocker는 planner substrate 수준에서 해결됐고, navmesh-only frame/nonblank gate와 detector substrate도 통과했다. 하지만 fixed post-observation rule은 own-view category evidence만으로 wrong candidates를 commit하므로 reject됐다. `goal_validity_arbitration_v1`은 같은 broader split에서 nonzero success와 zero wrong-goal을 동시에 만족하는 diagnostic repair였지만, independent/predeclared source에서는 safe-but-inert 결과를 냈다. Default counterfactual은 success를 회복하지만 wrong-goal도 만들기 때문에 같은 evidence의 threshold tuning은 reject한다. `discriminative_rival_view_planner_v1` v2 frame/nonblank and detector/SAM2 substrate gates pass, but `discriminative_rival_view_evidence_v1` fails because contrastive evidence still prefers the focus candidate on a single-correct rival case. Failure taxonomy shows cross-view leakage, common-view non-discrimination, invalid candidate pairs, and weak correct-rival own-view evidence. `request_expanded_retrieval` planner, label-join diagnostic, guard design, proxy feature extraction, source-pool validity proxy, detector standoff frame gate, projection-anchor implementation, fixed-anchor detector substrate, fresh source freeze, fresh proxy/plan/frame gates, fresh detector/SAM2 substrate, fresh detector evidence diagnostic, ambiguity-aware objective contract, paper-scale source freeze, paper-scale upper-anchor frame/projection smoke, paper-scale detector/SAM2 substrate, paper-scale detector evidence diagnostic, ambiguity objective application, local-context disambiguation contract, local-context planner smoke, local-context frame/projection smoke, local-context detector/SAM2 substrate, route-specific local-context analyzer, source-pool repair analyzer, backend pool expansion contract, backend pool expansion analyzer, backend candidate generation contract, backend candidate generation analyzer, deeper backend generation contract, deeper backend generation full job, recovered-row goal-validity confirmation contract, recovered-row request/branch analyzer, pool-validity branch fallback contract, pool-validity branch analyzer target-spec smoke, first fallback full job, second fallback contract, second fallback full job, candidate-specific goal-validity evidence planner, bounded frame/projection smoke, bounded detector/SAM2 substrate, and bounded objective analyzer pass or are frozen/completed on branch-level evidence. The local-context post-observation gate fails because own-view detector/SAM2 support still commits wrong/no-valid goals, and failure diagnosis separates no-valid pool failure from wrong-instance arbitration. Source-pool repair routes all five repair rows to backend expansion, backend expansion routes all five rows to fixed backend candidate generation because the top-10 preview is below the fixed budget minimum, and fixed top-20 generation leaves `3/5` no-valid pools after evaluation-only label join. Deeper backend generation recovers `2/3` of those rows, and the request/branch analyzer keeps recovered goal-validity evidence separate from the still-no-valid `rival_identity:13` backend/pool-validity branch. Both first and second fallback variants are structurally valid but still no-valid for `rival_identity:13`. The full candidate-specific detector/SAM2 substrate contract is now frozen for recovered rows `rival_identity:12` and `rival_identity:14`, so the current reproducible step is the full recovered-row frame/projection/detector run before the objective analyzer is rerun.
...
최신 dense path 기준으로는, 같은-goal correct cluster에서 성공한 local chair diagnostic을 확장 주장으로 쓰지 않는다. 현재 reproducible next step은 still-no-valid `rival_identity:13`에 대해 action-time에서 사용할 수 있는 pool-validity proxy 또는 fallback backend variant를 정의하는 것이다.
